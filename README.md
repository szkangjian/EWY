...
基于 `231,818` 条分钟数据、`522` 个交易日（`2024-03-25` 至 `2026-04-23`），所有数据统一为 `US/Eastern` 时区、仅使用常规交易时段 `09:30-16:00 ET`。

## 核心结论

- **IBS 是最强信号**：v2 参数 `IBS<0.45 / IBS>0.90`（0.05 步长精扫 + walk-forward 验证），`42` 笔交易，胜率 `81%`，总收益 `+93.6%`，均收益 `+2.23%/笔`。v1 (`0.20/0.80`) 总收益 `+37.2%`，被 v2 显著超越。
- **盘中跌幅反弹适合做辅助策略**：当前参数为 `-3.5% / +2.5% / 5 交易日 / 无止损`，`21` 笔、胜率 `71%`、总收益 `+15.9%`（2026-04-23 从 `-4.5%` 放宽，适配更高波动 regime）。
- **MA200 是风控开关**：加上 MA200 过滤后，跌幅策略表现显著改善，说明反弹信号对趋势环境敏感。
- **反弹效应依赖半导体/AI 强势 regime**：EWY 在强势周期中确实存在短线均值回归，但不能假设"什么原因跌都会反弹"。
- **最新一日观察**：`2026-04-23` 盘中跌幅策略触发（-3.5% 阈值），收盘前仍持仓；`IBS` 当天无新信号。
