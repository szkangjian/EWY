...
基于 `191,397` 条分钟数据、`500` 个交易日（`2024-03-25` 至 `2026-03-23`），所有数据统一为 `US/Eastern` 时区、仅使用常规交易时段 `09:30-16:00 ET`。

## 核心结论

- **IBS 是最强信号**：`28` 笔交易，胜率 `71%`，总收益 `+33.3%`，均收益 `+1.19%/笔`，平均持有 `3.3` 天。
- **盘中跌幅反弹适合做辅助策略**：参数 `-3.5% / +2.0% / 5天`，`21` 笔、胜率 `76%`、总收益 `+18.3%`，但有样本内优化风险。
